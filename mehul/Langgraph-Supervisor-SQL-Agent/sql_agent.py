...
query_check = query_check_prompt | ChatOpenAI(temperature=0).bind_tools(
...
model_get_schema = ChatOpenAI(temperature=0).bind_tools(
    [get_schema_tool]
)
...
query_gen = query_gen_prompt | ChatOpenAI(temperature=0).bind_tools(
    [SubmitFinalAnswer]
)
...
app = workflow.compile(name="SQL_Agent")
